...
class m6A_Dataset(Dataset):
    def __init__(self, csv_file):
        ...
        self.data = raw_data.view(-1, 51, 4)
...
class m6A_VAE(nn.Module):
    def __init__(self, bottleneck_dim=32):
        ...
        self.fc_encoder = nn.Sequential(
            nn.Linear(64 * 51, 512), # 672 -> 256 平滑过渡
            nn.ReLU()
        )
        
        # 连接到均值和方差
        self.fc_mu = nn.Linear(512, bottleneck_dim)      
        self.fc_logvar = nn.Linear(512, bottleneck_dim)  
        
        # 2. 解码器全连接部分（严格对称地还原）
        self.decoder_linear = nn.Sequential(
            nn.Linear(bottleneck_dim, 512),
            nn.ReLU(),
            nn.Linear(512, 64 * 51), # 256 -> 672
            nn.ReLU()
        )
        ...
        self.decoder_conv = nn.Sequential(
            ...
            nn.ConvTranspose1d(in_channels=32, out_channels=4, kernel_size=3, padding=1) 
        )
    ...
    def forward(self, x):
        ...
        dec = dec.view(-1, 64, 51)       # 变回图片格式 [Batch, 32, 21]
...
if __name__ == '__main__':
    # 1. 准备数据和模型
    csv_file_path = "m6A_51bp_onehot_features.csv" # 确保这里是你的正确文件名
    ...
    model = m6A_VAE(bottleneck_dim=32).to(device)
...
if __name__ == '__main__':
    # 1. 准备原始数据
    csv_file_path = "m6A_51bp_onehot_features.csv" 
    dataset = m6A_Dataset(csv_file_path)
    ...
    output_filename = "m6A_51bp_latent_features_32d.csv"
