...
class m6A_Dataset(Dataset):
    def __init__(self, csv_file):
        ...
        self.data = raw_data.view(-1, 101, 4)
...
class m6A_VAE(nn.Module):
    def __init__(self, bottleneck_dim=32):
        ...
        self.fc_encoder = nn.Sequential(
            nn.Linear(128 * 101, 1024), # 672 -> 256 平滑过渡
            nn.ReLU()
        )
        ...
        self.decoder_linear = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(1024, 128 * 101), # 256 -> 672
            nn.ReLU()
        )
        ...
        self.decoder_conv = nn.Sequential(
            ...
            nn.ConvTranspose1d(in_channels=64, out_channels=4, kernel_size=3, padding=1) 
        )
    ...
    def forward(self, x):
        ...
        dec = dec.view(-1, 128, 101)       # 变回图片格式 [Batch, 32, 21]
...
if __name__ == '__main__':
    # 1. 准备数据和模型
    csv_file_path = "m6A_101bp_onehot_features.csv" # 确保这里是你的正确文件名
...
if __name__ == '__main__':
    # 1. 准备原始数据
    csv_file_path = "m6A_101bp_onehot_features.csv" 
    dataset = m6A_Dataset(csv_file_path)
    ...
    output_filename = "m6A_101bp_latent_features_32d.csv"
