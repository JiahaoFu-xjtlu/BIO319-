...
raw_csv = pd.read_csv("m6A_21bp_onehot_features.csv", header=None, dtype=np.float32)
# 将其折叠为 4D 张量 (135300, 21, 4)
raw_data = raw_csv.values.reshape(-1, 21, 4)

print("⏳ 正在加载之前保存的聚类标签 (保证绝对一致性)...")
labels_df = pd.read_csv("m6A_21bp_UMAP_HDBSCAN_Labels.csv")
...
mapping_file = 'm6A_21bp_Sequence_Cluster_Mapping.csv'
...
plt.tight_layout()
output_name = 'm6A_21bp_Motif_Logos.png'
